...
function [p] = binplot_v2(x,y,n,c,sized) % s is shade (quartiles) and p is plot (median)
...
% Y = [y1 fliplr(y2)];
% s = fill(X,Y,c,'FaceAlpha',0.2,'EdgeAlpha',0.0);
hold on;  % !! x1,x2,y1 and y2 must be dim 1-n and 1-m (not n-1 and m-1)
p = plot(Median_xbins,Quartilesy_xbins(:,2),'LineStyle','none','LineWidth',1.5,'Marker','o','MarkerEdgeColor', ...
    c,'MarkerFaceColor','none','Color','none','MarkerSize',sized);
end